...
boolean DWHCIRootPortInitialize (TDWHCIRootPort *pThis)
{
  ...
  LogWrite (FromDWHCIRoot, LOG_ERROR, "port initializing");
	assert (pThis->m_pHost != 0);
	...
	LogWrite (FromDWHCIRoot, LOG_ERROR, "Got Speed");
  ...
  LogWrite (FromDWHCIRoot, LOG_ERROR, "Created USBDevice");
  LogWrite (FromDWHCIRoot, LOG_ERROR, pThis->m_pDevice->m_ManufacturerString.m_pString->m_pBuffer);
  ...
  LogWrite (FromDWHCIRoot, LOG_ERROR, "configuring");
