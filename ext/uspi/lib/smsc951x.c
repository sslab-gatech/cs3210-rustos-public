...
boolean SMSC951xDeviceSendFrame (TSMSC951xDevice *pThis, const void *pBuffer, unsigned nLength)
{
	...
	LogWrite ("SendFrame", LOG_DEBUG, "Send 1");
	...
	LogWrite ("SendFrame", LOG_DEBUG, "Send 2");

	assert (pThis->m_pTxBuffer != 0);
	assert (pBuffer != 0);
	memcpy (pThis->m_pTxBuffer+8, pBuffer, nLength);
	
	LogWrite ("SendFrame", LOG_DEBUG, "Send 3");
...
boolean SMSC951xDeviceReceiveFrame (TSMSC951xDevice *pThis, void *pBuffer, unsigned *pResultLength)
{
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 1");
	assert (pThis->m_pEndpointBulkIn != 0);
	assert (pBuffer != 0);

	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 2");

	TUSBRequest URB;
	USBRequest (&URB, pThis->m_pEndpointBulkIn, pBuffer, FRAME_BUFFER_SIZE, 0);
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 3");
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 4");

	u32 nResultLength = USBRequestGetResultLength (&URB);
	// should not happen with HW_CFG_BIR set
	if (nResultLength < 4)
	{
		_USBRequest (&URB);
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 5");
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 6");
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 7");
	...
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 8");

	assert (pResultLength != 0);
	*pResultLength = nFrameLength;
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 9");
	
	_USBRequest (&URB);
	//LogWrite ("RecvFrame", LOG_DEBUG, "Recv 10");
