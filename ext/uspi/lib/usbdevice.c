...
boolean USBDeviceInitialize (TUSBDevice *pThis)
{
  ...
  USBDeviceLogWrite (pThis, LOG_ERROR, "initializing");
  ...
  USBDeviceLogWrite (pThis, LOG_ERROR, "devicegetdescriptor");
  ...
  USBDeviceLogWrite (pThis, LOG_ERROR, "set max packetsize");
...
void USBDeviceLogWrite (TUSBDevice *pThis, unsigned Severity, const char *pMessage, ...)
{
	...
	DoLogWrite (StringGet (&Source), Severity, StringGet (&Message));
