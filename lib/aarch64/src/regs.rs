...
// (ref: D7.2.34: Hypervisor Configuration Register)
defreg!(
    HCR_EL2,
    [
...
// (ref: C5.2.5 Exception Link Register EL1)
defreg!(ELR_EL1);
// (ref: C5.2.6 Exception Link Register EL2)
defreg!(ELR_EL2);
...
// (ref: D7.2.25 Exception Syndrome Register)
defreg!(
    ESR_EL1,
    [
...
// (ref: D7.2.29-31 Fault Address Register)
defreg!(FAR_EL1);
defreg!(FAR_EL2);
defreg!(FAR_EL3);
...
// (ref: D5.2.10 Condition Flags)
defreg!(NZCV, [N[31 - 31], Z[30 - 30], C[29 - 29], V[28 - 28],]);
// (ref: D5.2.2 Interrupt Mask Bits)
defreg!(DAIF, [D[9 - 9], A[8 - 8], I[7 - 7], F[6 - 6],]);
...
defreg!(VBAR_EL1, [RES0[10 - 0],]);

// (ref: D7.5.2 Counter-timer Hypervisor Control Register)
defreg!(CNTHCTL_EL2, [EL0VCTEN[1 - 1], EL0PCTEN[0 - 0],]);

// (ref: D7.5.21 Counter-timer Virtual Offset Register)
defreg!(CNTVOFF_EL2);

// (ref: D7.5.1 Counter-timer Frequency Register)
defreg!(CNTFRQ_EL0);
